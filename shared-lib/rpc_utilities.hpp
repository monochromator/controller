 ...
 */
template <typename S, typename D> D rpc_receive(S &socket) {
  size_t received = 0;
  D data;
...
std::array<D, L> rpc_receive(S &socket) {
  ...
  do {
    auto chunk_received =
        socket.read(reinterpret_cast<uint8_t *>(data.data()) + received,
                    sizeof(D) * L - received);
 ...
 * @param D Data type
 */
template <typename S, typename D> void rpc_send(S &socket, D const &data) {
  size_t sent = 0;
